...
void initialTurbo()
{    
    pEnDataIn = (UWORD8 *)_aligned_malloc(MAX_K/8, 64);
    assert(pEnDataIn != NULL);
    pEnDataInBak = (UWORD8 *)_aligned_malloc(MAX_K/8, 64);
    assert(pEnDataInBak != NULL);
    pEnDataOut = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4)/8+1,64);
    assert(pEnDataOut != NULL);
    pEnOutD0 = (UWORD8 *)_aligned_malloc((MAX_K + 4)/8+1,64);
    assert(pEnOutD0 != NULL);
    pEnOutD1 = (UWORD8 *)_aligned_malloc((MAX_K + 4)/8+1,64);
    assert(pEnOutD1 != NULL);
    pEnOutD2 = (UWORD8 *)_aligned_malloc((MAX_K + 4)/8+1,64);
    assert(pEnOutD2 != NULL);
    //pRmOut = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4)/8+1,64);
    pModOut = (WORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
    assert(pModOut != NULL);
    pLLR = (WORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
    assert(pLLR != NULL);
    //pDescrambOut = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
    pDecOut = (UWORD8 *)_aligned_malloc(MAX_K/8, 64);
    assert(pDecOut != NULL);
    pTmpBuf1 = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
    assert(pTmpBuf1 != NULL);
    pTmpBuf2 = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
    assert(pTmpBuf2 != NULL);
    pTmpBuf3 = (UWORD8 *)_aligned_malloc(6528 * 16,64);
    assert(pTmpBuf3 != NULL);
    pTmpBuf4 = (UWORD8 *)_aligned_malloc(MAX_K,64);
    assert(pTmpBuf4 != NULL);
    pDeRmOut = (UWORD8 *)_aligned_malloc(3*(MAX_K + 4),64);
...
{    
    ...
    //float snr;
    UWORD64 evenNoise[3000][32]  __attribute__ ((aligned (64)));
    char noise[3000]  __attribute__ ((aligned (64)));
    WORD8 *pOut, *pIn;
    char *input;
    char c1, c2;
...
int main(int argc, char* argv[])
{
    ...
    UWORD16 K = atoi(argv[1]);
    float snrStart = atof(argv[2]);  /* ebno starting value for measurements */    
    float snrStep = 1.0f, snrStop = atof(argv[3]);    
    float snr = snrStart - snrStep; // init value

    initialTurbo();

    for(nsnrIdx = 0; nsnrIdx < snrStop - snrStart + 1; nsnrIdx++)
    {
        snr = snr + snrStep;
        ...
        for(nPktIdx = 0; nPktIdx < MAX_PKT_NUM; nPktIdx++)
        {
            ...
            //memset(pEnDataIn, 0xff, (K-CRC_LEN)>>3);///////
            ...
            /* 5. scramble */
            /*scramble_request.c_init = 0;
            scramble_request.data_in = pEnDataOut;
            scramble_request.len = (rate_match_request.G + 7) / 8;
            scramble_response.data_out = pRmOut;
            bblib_scramble(&scramble_request, &scramble_response);*/

            //print_byte(pRmOut, (rate_match_request.G + 7) / 8, "SCRAM OUT: ");
            ...
            /* 8. De-Scramble */
            /*descramble_request.data_in = (int16_t*)pModOut;//pLLR;
            descramble_request.is_discontiguous = 0;
            descramble_request.len = rate_match_request.G;
            descramble_request.c_init = 0;
            descramble_response.data_out = (int16_t*)pDescrambOut;
            bblib_descramble(&descramble_request, &descramble_response);*/

            //print_byte(pDescrambOut, rate_match_request.G, "DESCR OUT: ");
        ...
        for(int i = 0; i < MAX_WIN_NUM; i++)
        {
            PER[nsnrIdx][i] = (float)nErrPkts[i]/(float)MAX_PKT_NUM;
        }
    }
