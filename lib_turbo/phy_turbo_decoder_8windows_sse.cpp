...
{ \
    gamma1 = _mm_load_si128((__m128i *)ag_addr--);  \
    gamma0 = _mm_load_si128((__m128i *)ag_addr--);  \
    alpha = _mm_load_si128((__m128i *)ag_addr--); \
                           ...
                           int16_t *pTailAG, uint8_t *out_bit)
{
    ...
    //Calculate Alpha
    temp0 = _mm_setr_epi16(TURBR_INIT_OFFSET_SSE, -INF, -INF, -INF, -INF, -INF, -INF, -INF);
    ...
    struct bblib_turbo_decoder_response *response)
{
    ...
    uint16_t interleaverTable[6144];
    ...
    byte_print(0, pLLRout, "gama.txt", K);
    byte_print(0, tailLLRout, "gama_tail.txt", 3);
    ...
    for (j=0; j<numMaxIter; ++j)
    {
        ...
        byte_print(2 * j + 1, pLLRout, "gama.txt", K);
        byte_print(2 * j + 1, tailLLRout, "gama_tail.txt", 3);
        NumIter++;
        /* De-Interleaver */
        for (kIdx=0; kIdx<K; ++kIdx)
        {
        ...
        /* SISO1 */
        siso1_decoder(K, pSys, pLLRin, pLLRout, pAG, tailBits0, tailLLRin, tailLLRout, pTailAG, (uint8_t *)out_bit);
        byte_print(2 * j + 2, pLLRout, "gama.txt", K);
        byte_print(2 * j + 2, tailLLRout, "gama_tail.txt", 3);
