...
int16_t bblib_rate_match_version(char *version, int buffer_size) {
    ...
    const char *msg = "FlexRAN SDK bblib_lte_rate_matching version #DIRTY#";
...
int32_t bblib_rate_match_ul(const struct bblib_rate_match_ul_request *request,
        struct bblib_rate_match_ul_response *response, int nWinIdx) {
    return default_ul_func(request, response, nWinIdx);
}
...
int32_t bblib_turbo_adapter_ul(const struct bblib_turbo_adapter_ul_request *request,
        struct bblib_turbo_adapter_ul_response *response, int nWinIdx) {
    return default_turbo_adapter_func(request, response, nWinIdx);
}
